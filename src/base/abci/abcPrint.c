...
/**Function*************************************************************
  ...
  If the new network is not a logic network, quits without saving and returns
0.]
...
***********************************************************************/
int
Abc_NtkCompareAndSaveBest(Abc_Ntk_t* pNtk) {
    extern void Io_Write(Abc_Ntk_t * pNtk, char* pFileName,
    ...
    if (!Abc_NtkIsLogic(pNtk)) return 0;
    // get the parameters
    ...
    if (ParsBest.pName == NULL || strcmp(ParsBest.pName, pNtk->pName) ||
        ...
        // writ the network
        if (strcmp(pNtk->pSpec + strlen(pNtk->pSpec) - strlen("_best.blif"),
                   "_best.blif"))
            pFileNameOut =
                Extra_FileNameGenericAppend(pNtk->pSpec, "_best.blif");
        else pFileNameOut = pNtk->pSpec;
...
Abc_NtkMemory(Abc_Ntk_t* p) {
    ...
    Abc_NtkForEachObj(p, pObj, i) Memory +=
        sizeof(int) *
        (Vec_IntCap(&pObj->vFanins) + Vec_IntCap(&pObj->vFanouts));
    return Memory;
}
...
***********************************************************************/
float
Abc_NtkMfsTotalSwitching(Abc_Ntk_t* pNtk) {
    extern Aig_Man_t* Abc_NtkToDar(Abc_Ntk_t * pNtk, int fExors,
                                   int fRegisters);
    extern Vec_Int_t* Saig_ManComputeSwitchProbs(Aig_Man_t * p, int nFrames,
                                                 int nPref, int fProbOne);
    ...
    Abc_NtkForEachObj(
        pNtk, pObjAbc,
        i) if ((pObjAbc->pTemp &&
                Abc_ObjRegular((Abc_Obj_t*)pObjAbc->pTemp)->Type ==
                    ABC_FUNC_NONE) ||
               (!Abc_ObjIsCi(pObjAbc) && !Abc_ObjIsNode(pObjAbc)))
        pObjAbc->pTemp = NULL;
    ...
    Abc_NtkForEachObj(pNtk, pObjAbc, i) {
        if ((pObjAbc2 = Abc_ObjRegular((Abc_Obj_t*)pObjAbc->pTemp)) &&
            (pObjAig = Aig_Regular((Aig_Obj_t*)pObjAbc2->pTemp))) {
            Result += Abc_ObjFanoutNum(pObjAbc) * pSwitching[pObjAig->Id];
            //            Abc_ObjPrint( stdout, pObjAbc );
            //            printf( "%d = %.2f\n", i, Abc_ObjFanoutNum(pObjAbc) *
            //            pSwitching[pObjAig->Id] );
        }
    }
...
***********************************************************************/
void
Abc_NtkPrintStats(Abc_Ntk_t* pNtk, int fFactored, int fSaveBest,
                  int fDumpResult, int fUseLutLib, int fPrintMuxes, int fPower,
                  int fGlitch, int fSkipBuf, int fSkipSmall, int fPrintMem) {
    ...
    if (fPrintMuxes && Abc_NtkIsStrash(pNtk)) {
        ...
        Abc_Print(1, "Xor =%7d (%6.2f %%)  ", nXors,
                  300.0 * nXors / Abc_NtkNodeNum(pNtk));
        Abc_Print(1, "Mux =%7d (%6.2f %%)  ", nMuxs,
                  300.0 * nMuxs / Abc_NtkNodeNum(pNtk));
        Abc_Print(1, "And =%7d (%6.2f %%)  ", nAnds,
                  100.0 * nAnds / Abc_NtkNodeNum(pNtk));
        Abc_Print(1, "Total =%7d", nAnds + nXors + nMuxs);
        Abc_Print(1, "\n");
        return;
    }
    if (fSaveBest) Abc_NtkCompareAndSaveBest(pNtk);
        /*
            if ( fDumpResult )
            {
                char Buffer[1000] = {0};
                const char * pNameGen = pNtk->pSpec? Extra_FileNameGeneric(
           pNtk->pSpec ) : "nameless_"; sprintf( Buffer, "%s_dump.blif",
           pNameGen ); Io_Write( pNtk, Buffer, IO_FILE_BLIF ); if ( pNtk->pSpec
           ) ABC_FREE( pNameGen );
            }
        */
    ...
    if (Abc_NtkConstrNum(pNtk)) Abc_Print(1, "(c=%d)", Abc_NtkConstrNum(pNtk));
    Abc_Print(1, "  lat =%5d", Abc_NtkLatchNum(pNtk));
    if (pNtk->nBarBufs) Abc_Print(1, "(b=%d)", pNtk->nBarBufs);
    if (Abc_NtkIsNetlist(pNtk)) {
        Abc_Print(1, "  net =%5d", Abc_NtkNetNum(pNtk));
        Abc_Print(1, "  nd =%5d",
                  fSkipSmall ? Abc_NtkGetLargeNodeNum(pNtk)
                             : Abc_NtkNodeNum(pNtk) - nSingles);
    ...
    if (Abc_NtkIsStrash(pNtk) || Abc_NtkIsNetlist(pNtk)) {
        ...
        if (fFactored)
            Abc_Print(1, "  lit(fac) =%6d",
                      Abc_NtkGetLitFactNum(pNtk) - nSingles);
    ...
    if (Abc_NtkIsStrash(pNtk)) {
        ...
        //        Abc_Print( 1,"  ff = %5d", Abc_NtkNodeNum(pNtk) + 2 *
        //        (Abc_NtkCoNum(pNtk)+Abc_NtkGetMultiRefNum(pNtk)) ); Abc_Print(
        //        1,"  var = %5d", Abc_NtkCiNum(pNtk) +
        //        Abc_NtkCoNum(pNtk)+Abc_NtkGetMultiRefNum(pNtk) );
    } else Abc_Print(1, "  lev = %d", Abc_NtkLevel(pNtk));
    if (pNtk->nBarBufs2) Abc_Print(1, "  buf = %d", pNtk->nBarBufs2);
    ...
    if (fPower) Abc_Print(1, "  power =%7.2f", Abc_NtkMfsTotalSwitching(pNtk));
    if (fGlitch) {
        if (Abc_NtkIsLogic(pNtk) && Abc_NtkGetFaninMax(pNtk) <= 6)
            Abc_Print(1, "  glitch =%7.2f %%",
                      Abc_NtkMfsTotalGlitching(pNtk, 4000, 8, 0));
        else
    ...
    */
    ...
    //        fprintf( pTable, "%.2f ", (float)(s_MappingMem)/(float)(1<<20) );
            fprintf( pTable, "%.2f",
    (float)(s_MappingTime)/(float)(CLOCKS_PER_SEC) );
    //        fprintf( pTable, "%.2f",
    (float)(s_ResynTime)/(float)(CLOCKS_PER_SEC) ); fprintf( pTable, "\n" );
            fclose( pTable );
    ...
    */
    ...
    //        fprintf( pTable, "%d ", Abc_NtkLatchNum(pNtk) );
            fprintf( pTable, "%d ", Abc_NtkNodeNum(pNtk) );
            fprintf( pTable, "%.2f ",
    (float)(timeRetime)/(float)(CLOCKS_PER_SEC) ); fprintf( pTable, "\n" );
            fclose( pTable );
        }
    ...
    */
    ...
    if (0) {
        ...
        if (Abc_NtkIsStrash(pNtk)) fprintf(pTable, "%s ", pNtk->pName);
        fprintf(pTable, "%d ", Abc_NtkNodeNum(pNtk));
        fclose(pTable);
    }
    ...
    fflush(stdout);
    if (pNtk->pExdc)
        Abc_NtkPrintStats(pNtk->pExdc, fFactored, fSaveBest, fDumpResult,
                          fUseLutLib, fPrintMuxes, fPower, fGlitch, fSkipBuf,
                          fSkipSmall, fPrintMem);
}
...
***********************************************************************/
void
Abc_NtkPrintIo(FILE* pFile, Abc_Ntk_t* pNtk, int fPrintFlops) {
    Abc_Obj_t* pObj;
    int        i;
    ...
    //        fprintf( pFile, " %s(%d)", Abc_ObjName(pObj),
    ...
    if (!fPrintFlops) return;

    fprintf(pFile, "Latches (%d):  ", Abc_NtkLatchNum(pNtk));
    Abc_NtkForEachLatch(pNtk, pObj, i) fprintf(
        pFile, " %s(%s=%s)", Abc_ObjName(pObj),
...
***********************************************************************/
void
Abc_NtkPrintLatch(FILE* pFile, Abc_Ntk_t* pNtk) {
    ...
    Abc_NtkForEachLatch(pNtk, pLatch, i) {
        ...
        if (Abc_NtkIsLogic(pNtk)) {
            if (!Abc_NodeIsConst(pFanin)) continue;
        } else if (Abc_NtkIsStrash(pNtk)) {
            if (!Abc_AigNodeIsConst(pFanin)) continue;
        } else assert(0);
        ...
        // count the number of cases when the constant is equal to the initial
        // value
        if (Abc_NtkIsStrash(pNtk)) {
            if (Abc_LatchIsInit1(pLatch) == !Abc_ObjFaninC0(pLatch)) Counter2++;
        } else {
            if (Abc_LatchIsInit1(pLatch) ==
    ...
    fprintf(pFile,
            "Total latches = %5d. Init0 = %d. Init1 = %d. InitDC = %d. Const "
            "data = %d.\n",
            Abc_NtkLatchNum(pNtk), InitNums[1], InitNums[2], InitNums[3],
            Counter0);
    //    fprintf( pFile, "Const fanin = %3d. DC init = %3d. Matching init =
    //    %3d. ", Counter0, Counter1, Counter2 ); fprintf( pFile, "Self-feed
...
***********************************************************************/
void
Abc_NtkFaninFanoutCounters(Abc_Ntk_t* pNtk, Vec_Int_t* vFan, Vec_Int_t* vFon,
                           Vec_Int_t* vFanR, Vec_Int_t* vFonR) {
    Abc_Obj_t* pNode;
...
void
Abc_NtkInputOutputCounters(Abc_Ntk_t* pNtk, Vec_Int_t* vFan, Vec_Int_t* vFon,
                           Vec_Int_t* vFanR, Vec_Int_t* vFonR) {
    Abc_Obj_t* pNode;
    int        i, nFanins, nFanouts;
    int        nFaninsMax = 0, nFanoutsMax = 0;
    Abc_NtkForEachCi(pNtk, pNode, i) nFanoutsMax =
        Abc_MaxInt(nFanoutsMax, Abc_ObjFanoutNum(pNode));
    Abc_NtkForEachCo(pNtk, pNode, i) nFaninsMax =
        Abc_MaxInt(nFaninsMax, Abc_ObjFaninNum(Abc_ObjFanin0(pNode)));
    Vec_IntFill(vFan, nFaninsMax + 1, 0);
    Vec_IntFill(vFon, nFanoutsMax + 1, 0);
    Vec_IntFill(vFanR, nFaninsMax + 1, 0);
    Vec_IntFill(vFonR, nFanoutsMax + 1, 0);
...
Vec_Int_t*
Abc_NtkCollectCoSupps(Abc_Ntk_t* pNtk, int fVerbose) {
    ...
    Abc_NtkForEachCo(pNtk, pNode, i) Vec_IntPush(
        vSuppsCo, Vec_IntSize(Vec_WecEntry(vSupps, Abc_ObjFaninId0(pNode))));
    Vec_WecFree(vSupps);
    Vec_PtrFree(vNodes);
    Vec_IntFree(vTemp);
    if (fVerbose)
...
Vec_Int_t*
Abc_NtkCollectCiSupps(Abc_Ntk_t* pNtk, int fVerbose) {
    abctime    clk = Abc_Clock();
    Abc_Obj_t* pNode;
    int        i, k;
    Vec_Ptr_t* vNodes = Abc_NtkDfs(pNtk, 0);
    ...
    Abc_NtkForEachCi(pNtk, pNode, i) Vec_IntPush(
        vSuppsCi, Vec_IntSize(Vec_WecEntry(vSupps, Abc_ObjId(pNode))));
    Vec_WecFree(vSupps);
    Vec_PtrFree(vNodes);
    Vec_IntFree(vTemp);
    if (fVerbose)
...
void
Abc_NtkInOutSupportCounters(Abc_Ntk_t* pNtk, Vec_Int_t* vFan, Vec_Int_t* vFon,
                            Vec_Int_t* vFanR, Vec_Int_t* vFonR) {
    Abc_Obj_t* pNode;
...
Vec_Int_t*
Abc_NtkCollectCoCones(Abc_Ntk_t* pNtk, int fVerbose) {
    abctime    clk = Abc_Clock();
    Abc_Obj_t* pNode;
    int        i, k;
    Vec_Ptr_t* vNodes = Abc_NtkDfs(pNtk, 0);
...
Vec_Int_t*
Abc_NtkCollectCiCones(Abc_Ntk_t* pNtk, int fVerbose) {
    abctime    clk = Abc_Clock();
    Abc_Obj_t* pNode;
    int        i, k;
    Vec_Ptr_t* vNodes = Abc_NtkDfs(pNtk, 0);
...
void
Abc_NtkInOutConeCounters(Abc_Ntk_t* pNtk, Vec_Int_t* vFan, Vec_Int_t* vFon,
                         Vec_Int_t* vFanR, Vec_Int_t* vFonR) {
    Abc_Obj_t* pNode;
...
void
Abc_NtkPrintDistribInternal(FILE* pFile, Abc_Ntk_t* pNtk, char* pFanins,
                            char* pFanouts, char* pNode, char* pFanin,
                            char* pFanout, Vec_Int_t* vFan, Vec_Int_t* vFon,
                            Vec_Int_t* vFanR, Vec_Int_t* vFonR) {
    ...
    for (k = 0; k < nSizeMax; k++) {
        ...
        if (EntryFan == 0 && EntryFon == 0) continue;

        fprintf(pFile, "%5d : ", k);
        if (EntryFan == 0) fprintf(pFile, "              ");
        else fprintf(pFile, "%12d  ", EntryFan);
        fprintf(pFile, "    ");
        if (EntryFon == 0) fprintf(pFile, "              ");
        else fprintf(pFile, "%12d  ", EntryFon);
...
void
Abc_NtkPrintFanio(FILE* pFile, Abc_Ntk_t* pNtk, int fUseFanio, int fUsePio,
                  int fUseSupp, int fUseCone) {
    ...
    if (fUseFanio) {
        ...
        Abc_NtkPrintDistribInternal(pFile, pNtk, "fanins", "fanouts", "Nodes",
                                    "fanin", "fanout", vFan, vFon, vFanR,
                                    vFonR);
    } else if (fUsePio) {
        Abc_NtkInputOutputCounters(pNtk, vFan, vFon, vFanR, vFonR);
        Abc_NtkPrintDistribInternal(pFile, pNtk, "fanins", "fanouts", "I/O",
                                    "fanin", "fanout", vFan, vFon, vFanR,
                                    vFonR);
    } else if (fUseSupp) {
        Abc_NtkInOutSupportCounters(pNtk, vFan, vFon, vFanR, vFonR);
        Abc_NtkPrintDistribInternal(pFile, pNtk, "input supports",
                                    "output supports", "I/O", "in-supp",
                                    "out-supp", vFan, vFon, vFanR, vFonR);
    } else if (fUseCone) {
        Abc_NtkInOutConeCounters(pNtk, vFan, vFon, vFanR, vFonR);
        Abc_NtkPrintDistribInternal(pFile, pNtk, "input cones", "output cones",
                                    "I/O", "in-cone", "out-cone", vFan, vFon,
                                    vFanR, vFonR);
    }
    Vec_IntFree(vFan);
    Vec_IntFree(vFon);
...
***********************************************************************/
void
Abc_NtkPrintFanioNew(FILE* pFile, Abc_Ntk_t* pNtk, int fMffc) {
    char       Buffer[100];
    Abc_Obj_t* pNode;
    ...
    Abc_NtkForEachNode(pNtk, pNode, i) {
        if (fMffc && Abc_ObjFanoutNum(pNode) == 1) continue;
        nFanins = Abc_ObjFaninNum(pNode);
        if (Abc_NtkIsNetlist(pNtk))
            nFanouts = Abc_ObjFanoutNum(Abc_ObjFanout0(pNode));
        else if (fMffc) nFanouts = Abc_NodeMffcSize(pNode);
        else nFanouts = Abc_ObjFanoutNum(pNode);
        nFaninsAll += nFanins;
    ...
    nSizeMax = Abc_MaxInt(10 * (Abc_Base10Log(nFaninsMax) + 1),
                          10 * (Abc_Base10Log(nFanoutsMax) + 1));
    ...
    Abc_NtkForEachNode(pNtk, pNode, i) {
        ...
        if (nFanins < 10) Vec_IntAddToEntry(vFanins, nFanins, 1);
        else if (nFanins < 100)
        ...
        if (nFanouts < 10) Vec_IntAddToEntry(vFanouts, nFanouts, 1);
    ...
    for (k = 0; k < nSizeMax; k++) {
        if (vFanins->pArray[k] == 0 && vFanouts->pArray[k] == 0) continue;
        if (k < 10) fprintf(pFile, "%15d : ", k);
        else {
            sprintf(Buffer, "%d - %d", (int)pow((double)10, k / 10) * (k % 10),
                    (int)pow((double)10, k / 10) * (k % 10 + 1) - 1);
            fprintf(pFile, "%15s : ", Buffer);
        }
        if (vFanins->pArray[k] == 0) fprintf(pFile, "              ");
        else fprintf(pFile, "%12d  ", vFanins->pArray[k]);
        fprintf(pFile, "    ");
        if (vFanouts->pArray[k] == 0) fprintf(pFile, "              ");
        else fprintf(pFile, "%12d  ", vFanouts->pArray[k]);
        fprintf(pFile, "\n");
    }
    Vec_IntFree(vFanins);
    ...
    fprintf(pFile,
            ...
            1.0 * nFanoutsAll / Abc_NtkNodeNum(pNtk));
    /*
        Abc_NtkForEachCi( pNtk, pNode, i )
        {
...
***********************************************************************/
void
Abc_NodePrintFanio(FILE* pFile, Abc_Obj_t* pNode) {
    Abc_Obj_t* pNode2;
    int        i;
    if (Abc_ObjIsPo(pNode)) pNode = Abc_ObjFanin0(pNode);
...
***********************************************************************/
void
Abc_NtkPrintMffc(FILE* pFile, Abc_Ntk_t* pNtk) {
    Abc_Obj_t*  pNode;
    int         i;
    extern void Abc_NodeMffcConeSuppPrint(Abc_Obj_t * pNode);
    Abc_NtkForEachNode(pNtk, pNode, i) if (Abc_ObjFanoutNum(pNode) > 1)
...
***********************************************************************/
void
Abc_NodePrintFactor(FILE* pFile, Abc_Obj_t* pNode, int fUseRealNames) {
    Dec_Graph_t* pGraph;
    Vec_Ptr_t*   vNamesIn;
    if (Abc_ObjIsCo(pNode)) pNode = Abc_ObjFanin0(pNode);
    ...
    if (fUseRealNames) {
        ...
        Dec_GraphPrint(stdout, pGraph, (char**)vNamesIn->pArray,
                       Abc_ObjName(pNode));
        Abc_NodeFreeNames(vNamesIn);
    } else Dec_GraphPrint(stdout, pGraph, (char**)NULL, Abc_ObjName(pNode));
    Dec_GraphFree(pGraph);
}
void
Abc_NtkPrintFactor(FILE* pFile, Abc_Ntk_t* pNtk, int fUseRealNames) {
    Abc_Obj_t* pNode;
    int        i;
...
***********************************************************************/
void
Abc_NodePrintSop(FILE* pFile, Abc_Obj_t* pNode, int fUseRealNames) {
    ...
    if (!Abc_SopGetPhase(pSop)) fprintf(pFile, "!");
    fprintf(pFile, "%s = ", Abc_ObjName(pNode));
    if (fUseRealNames) vNamesIn = Abc_NodeGetFaninNames(pNode);
    Abc_SopForEachCube(pSop, nVars, pCube) {
        if (pCube != pSop) fprintf(pFile, " +");
        if (vNamesIn) {
            for (pCur = pCube; *pCur != ' '; pCur++)
                if (*pCur != '-')
                    fprintf(pFile, " %s%s", *pCur == '0' ? "!" : "",
                            (char*)Vec_PtrEntry(vNamesIn, pCur - pCube));
        } else {
            for (pCur = pCube; *pCur != ' '; pCur++)
                if (*pCur != '-')
                    fprintf(pFile, " %s%c", *pCur == '0' ? "!" : "",
                            (char)('a' + pCur - pCube));
        }
    }
    fprintf(pFile, "\n");
...
void
Abc_NtkPrintSop(FILE* pFile, Abc_Ntk_t* pNtk, int fUseRealNames) {
    Abc_Obj_t* pNode;
    int        i;
    assert(Abc_NtkIsSopLogic(pNtk));
    Abc_NtkForEachNode(pNtk, pNode, i)
...
Abc_NtkPrintLevel(FILE* pFile, Abc_Ntk_t* pNtk, int fProfile, int fListNodes,
    ...
    if (fProfile && Abc_NtkHasMapping(pNtk)) {
        ...
        Abc_NtkForEachCo(pNtk, pNode, i) {
            if (Abc_ObjIsNode(Abc_ObjFanin0(pNode)) &&
                Abc_ObjFaninNum(Abc_ObjFanin0(pNode)) == 0)
                DelayInt = 0;
            else {
                DelayCur = Abc_NodeReadArrivalWorst(Abc_ObjFanin0(pNode));
                DelayInt = (int)(DelayCur / DelayDelta);
                if (DelayInt >= nIntervals) DelayInt = nIntervals - 1;
            }
        ...
        for (i = 0; i < nIntervals; i++) {
            ...
            printf("[%8.2f - %8.2f] :   COs = %4d.   %5.1f %%\n",
                   DelayDelta * i, DelayDelta * (i + 1), pLevelCounts[i],
                   100.0 * nOutsSum / nOutsTotal);
        }
        ...
        if (!Abc_NtkIsStrash(pNtk)) Abc_NtkLevel(pNtk);

        LevelMax = 0;
        Abc_NtkForEachCo(pNtk, pNode,
                         i) if (LevelMax < (int)Abc_ObjFanin0(pNode)->Level)
        ...
        for (i = 0; i <= LevelMax; i++)
            if (pLevelCounts[i]) {
                ...
                printf("Level = %4d.  COs = %4d.   %5.1f %%\n", i,
                       pLevelCounts[i], 100.0 * nOutsSum / nOutsTotal);
            }
        ABC_FREE(pLevelCounts);
        return;
    }
    ...
    if (fVerbose) {
        ...
        Length = 0;
        Abc_NtkForEachCo(pNtk, pNode,
                         i) if (Length < (int)strlen(Abc_ObjName(pNode)))
...
***********************************************************************/
void
Abc_NodePrintLevel(FILE* pFile, Abc_Obj_t* pNode) {
    Abc_Obj_t* pDriver;
    Vec_Ptr_t* vNodes;
...
***********************************************************************/
void
Abc_NodePrintKMap(Abc_Obj_t* pNode, int fUseRealNames) {
#ifdef ABC_USE_CUDD
    Vec_Ptr_t* vNamesIn;
    if (fUseRealNames) {
        vNamesIn = Abc_NodeGetFaninNames(pNode);
        Extra_PrintKMap(stdout, (DdManager*)pNode->pNtk->pManFunc,
                        (DdNode*)pNode->pData, Cudd_Not(pNode->pData),
                        Abc_ObjFaninNum(pNode), NULL, 0,
...
***********************************************************************/
void
Abc_NtkPrintGates(Abc_Ntk_t* pNtk, int fUseLibrary, int fUpdateProfile) {
    Abc_Obj_t* pObj;
    int        fHasBdds, i;
    int        CountConst, CountBuf, CountInv, CountAnd, CountOr, CountOther,
        CounterTotal, TotalDiff = 0;
    char* pSop;
    ...
    if (fUseLibrary && Abc_NtkHasMapping(pNtk)) {
        ...
        for (i = 0; i < nGates; i++) {
            ...
            if (fUpdateProfile) Mio_GateSetProfile2(ppGates[i], 0);
        }
        ...
        Abc_NtkForEachNodeNotBarBuf(pNtk, pObj, i) {
            if (i == 0) continue;
            Mio_GateSetValue((Mio_Gate_t*)pObj->pData,
                             1 + Mio_GateReadValue((Mio_Gate_t*)pObj->pData));
            if (fUpdateProfile) Mio_GateIncProfile2((Mio_Gate_t*)pObj->pData);
            CounterTotal++;
            // assuming that twin gates follow each other
            if (Abc_NtkFetchTwinNode(pObj)) i++;
        }
        ...
        for (i = 0; i < nGates; i++) {
            ...
            nGateNameLen =
                Abc_MaxInt(nGateNameLen, strlen(Mio_GateReadName(ppGates[i])));
        }
        ...
        for (i = 0; i < nGates; i++) {
            Counter = Mio_GateReadValue(ppGates[i]);
            if (Counter == 0 && Mio_GateReadProfile(ppGates[i]) == 0) continue;
            if (Mio_GateReadPinNum(ppGates[i]) > 1)
                TotalDiff += Abc_AbsInt(Mio_GateReadProfile(ppGates[i]) -
                                        Mio_GateReadProfile2(ppGates[i]));
            Area = Counter * Mio_GateReadArea(ppGates[i]);
            printf("%-*s   Fanin = %2d   Instance = %8d   Area = %10.2f   "
                   "%6.2f %%   %8d  %8d   %s\n",
                   nGateNameLen, Mio_GateReadName(ppGates[i]),
                   Mio_GateReadPinNum(ppGates[i]), Counter, Area,
                   100.0 * Area / AreaTotal, Mio_GateReadProfile(ppGates[i]),
                   Mio_GateReadProfile2(ppGates[i]),
                   Mio_GateReadForm(ppGates[i]));
        }
        printf("%-*s                Instance = %8d   Area = %10.2f   %6.2f %%  "
               " AbsDiff = %d\n",
               nGateNameLen, "TOTAL", CounterTotal, AreaTotal, 100.0,
               TotalDiff);
        return;
    }

    if (Abc_NtkIsAigLogic(pNtk)) return;
    ...
    CountConst = CountBuf = CountInv = CountAnd = CountOr = CountOther =
        CounterTotal                                      = 0;
    Abc_NtkForEachNodeNotBarBuf(pNtk, pObj, i) {
        if (i == 0) continue;
        if (Abc_NtkHasMapping(pNtk))
            pSop = Mio_GateReadSop((Mio_Gate_t*)pObj->pData);
        else pSop = (char*)pObj->pData;
        // collect the stats
        if (Abc_SopIsConst0(pSop) || Abc_SopIsConst1(pSop)) CountConst++;
        else if (Abc_SopIsBuf(pSop)) CountBuf++;
        else if (Abc_SopIsInv(pSop)) CountInv++;
    ...
    printf("Const        = %8d    %6.2f %%\n", CountConst,
           100.0 * CountConst / CounterTotal);
    printf("Buffer       = %8d    %6.2f %%\n", CountBuf,
           100.0 * CountBuf / CounterTotal);
    printf("Inverter     = %8d    %6.2f %%\n", CountInv,
           100.0 * CountInv / CounterTotal);
    printf("And          = %8d    %6.2f %%\n", CountAnd,
           100.0 * CountAnd / CounterTotal);
    printf("Or           = %8d    %6.2f %%\n", CountOr,
           100.0 * CountOr / CounterTotal);
    printf("Other        = %8d    %6.2f %%\n", CountOther,
           100.0 * CountOther / CounterTotal);
    printf("TOTAL        = %8d    %6.2f %%\n", CounterTotal,
           100.0 * CounterTotal / CounterTotal);

    // convert the network back into BDDs if this is how it was
    if (fHasBdds) Abc_NtkSopToBdd(pNtk);
}
...
***********************************************************************/
void
Abc_NtkPrintSharing(Abc_Ntk_t* pNtk) {
    ...
    Abc_NtkForEachCo(pNtk, pObj1, i) {
        ...
        // mark the nodes
        Vec_PtrForEachEntry(Abc_Obj_t*, vNodes1, pNode1, m) pNode1->fMarkA = 1;
        ...
        Abc_NtkForEachCo(pNtk, pObj2, k) {
            ...
            Counter = 0;
            Vec_PtrForEachEntry(Abc_Obj_t*, vNodes2, pNode2, n) Counter +=
                pNode2->fMarkA;
            // print
        ...
        // unmark the nodes
        Vec_PtrForEachEntry(Abc_Obj_t*, vNodes1, pNode1, m) pNode1->fMarkA = 0;
        Vec_PtrFree(vNodes1);
    }
    printf("\n");
}
...
***********************************************************************/
int
Abc_NtkCountPis(Vec_Ptr_t* vSupp) {
    Abc_Obj_t* pObj;
    int        i, Counter = 0;
    Vec_PtrForEachEntry(Abc_Obj_t*, vSupp, pObj, i) Counter +=
        Abc_ObjIsPi(pObj);
    return Counter;
}
void
Abc_NtkPrintStrSupports(Abc_Ntk_t* pNtk, int fMatrix) {
    ...
    Abc_NtkForEachCi(pNtk, pObj, k) pObj->fMarkA = 0;

    printf("Actual support info:\n");
    Abc_NtkForEachCo(pNtk, pObj, i) {
        vSupp = Abc_NtkNodeSupport(pNtk, &pObj, 1);
        Vec_PtrForEachEntry(Abc_Obj_t*, vSupp, pObj, k) pObj->fMarkA = 1;
        Vec_PtrFree(vSupp);

        Abc_NtkForEachCi(pNtk, pObj, k) printf("%d", pObj->fMarkA);
        printf("\n");
...
***********************************************************************/
void
Abc_ObjPrint(FILE* pFile, Abc_Obj_t* pObj) {
    Abc_Obj_t* pFanin;
    int        i;
    ...
    Abc_ObjForEachFanin(pObj, pFanin, i) fprintf(pFile, "%d ", pFanin->Id);
    fprintf(pFile, ") ");
    /*
        fprintf( pFile, " Fanouts ( " );
        Abc_ObjForEachFanout( pObj, pFanin, i )
            fprintf( pFile, "%d(%c) ", pFanin->Id,
       Abc_NodeIsTravIdCurrent(pFanin)? '+' : '-' ); fprintf( pFile, ") " );
    */
...
***********************************************************************/
void
Abc_NtkPrintMiter(Abc_Ntk_t* pNtk) {
    ...
    int        nUndec = 0;
    int        nPis   = 0;
    Abc_NtkForEachPi(pNtk, pObj, i) nPis += (int)(Abc_ObjFanoutNum(pObj) > 0);
    Abc_NtkForEachPo(pNtk, pObj, i) {
        pChild = Abc_ObjChild0(pObj);
        // check if the output is constant 0
        if (pChild == Abc_ObjNot(pConst1)) nUnsat++;
...
***********************************************************************/
void
Abc_NtkPrintPoEquivs(Abc_Ntk_t* pNtk) {
    Abc_Obj_t *pObj, *pDriver, *pRepr;
    int        i, iRepr;
    ...
    Abc_NtkForEachPo(pNtk, pObj, i) {
        ...
        printf("%s = %s%s\n", Abc_ObjName(pObj),
               Abc_ObjFaninC0(pRepr) == Abc_ObjFaninC0(pObj) ? "" : "!",
               Abc_ObjName(pRepr));
    }
    Vec_IntFree(vMap);
}
...
extern int Gli_ManCreateNode(Gli_Man_t* p, Vec_Int_t* vFanins, int nFanouts,
                             word* pGateTruth);

extern void Gli_ManSwitchesAndGlitches(Gli_Man_t* p, int nPatterns,
                                       float PiTransProb, int fVerbose);
...
***********************************************************************/
float
Abc_NtkMfsTotalGlitchingLut(Abc_Ntk_t* pNtk, int nPats, int Prob,
                            int fVerbose) {
    ...
    if (Abc_NtkGetFaninMax(pNtk) > 6) {
        printf("Abc_NtkMfsTotalGlitching() This procedure works only for "
               "mapped networks with LUTs size up to 6 inputs.\n");
        return -1.0;
    }
    Abc_NtkToAig(pNtk);
    ...
    p = Gli_ManAlloc(Vec_PtrSize(vNodes) + Abc_NtkCiNum(pNtk) +
                         Abc_NtkCoNum(pNtk),
                     Abc_NtkLatchNum(pNtk),
                     Abc_NtkGetTotalFanins(pNtk) + Abc_NtkCoNum(pNtk));
    Abc_NtkForEachObj(pNtk, pObj, i) pObj->iTemp = -1;
    Abc_NtkForEachCi(pNtk, pObj, i) pObj->iTemp =
        Gli_ManCreateCi(p, Abc_ObjFanoutNum(pObj));
    Vec_PtrForEachEntry(Abc_Obj_t*, vNodes, pObj, i) {
        Vec_IntClear(vFanins);
        Abc_ObjForEachFanin(pObj, pFanin, k)
            Vec_IntPush(vFanins, pFanin->iTemp);
        puTruth = Hop_ManConvertAigToTruth((Hop_Man_t*)pNtk->pManFunc,
                                           (Hop_Obj_t*)pObj->pData,
                                           Abc_ObjFaninNum(pObj), vTruth, 0);
        pTruth  = Vec_WrdEntryP(vTruths, Abc_ObjId(pObj));
        *pTruth = ((word)puTruth[Abc_ObjFaninNum(pObj) == 6] << 32) |
                  (word)puTruth[0];
        pObj->iTemp =
            Gli_ManCreateNode(p, vFanins, Abc_ObjFanoutNum(pObj), pTruth);
    }
    Abc_NtkForEachCo(pNtk, pObj, i)
    ...
    Abc_NtkForEachObj(pNtk, pObj, i) if (pObj->iTemp >= 0) {
        nSwitches +=
            Abc_ObjFanoutNum(pObj) * Gli_ObjNumSwitches(p, pObj->iTemp);
        nGlitches +=
            Abc_ObjFanoutNum(pObj) * Gli_ObjNumGlitches(p, pObj->iTemp);
    }
...
***********************************************************************/
float
Abc_NtkMfsTotalGlitching(Abc_Ntk_t* pNtk, int nPats, int Prob, int fVerbose) {
    int        nSwitches, nGlitches;
    Gli_Man_t* p;
    Vec_Ptr_t* vNodes;
    Vec_Int_t* vFanins;
    ...
    Vec_PtrForEachEntry(Abc_Obj_t*, vNodes, pObj, i) {
        ...
        Abc_ObjForEachFanin(pObj, pFanin, k)
            Vec_IntPush(vFanins, pFanin->iTemp);
        pObj->iTemp =
            Gli_ManCreateNode(p, vFanins, Abc_ObjFanoutNum(pObj),
                              Mio_GateReadTruthP((Mio_Gate_t*)pObj->pData));
    }
    Abc_NtkForEachCo(pNtk, pObj, i)
        Gli_ManCreateCo(p, Abc_ObjFanin0(pObj)->iTemp);
...
***********************************************************************/
void
Abc_Show6VarFunc(word F0, word F1) {
    ...
    for (s = 0; s < 4; s++) printf(" ");
    printf(" ");
    for (h = 0; h < 8; h++) {
        for (s = 0; s < 3; s++) printf("%d", ((Cells[h] >> (2 - s)) & 1));
        printf(" ");
    }
    printf("\n");
    ...
    for (v = 0; v < 8; v++) {
        for (s = 0; s < 3; s++) printf("%d", ((Cells[v] >> (2 - s)) & 1));
        printf(" |");

        for (h = 0; h < 8; h++) {
            printf(" ");
            if (((F0 >> ((Cells[v] * 8) + Cells[h])) & 1)) printf("0");
            else if (((F1 >> ((Cells[v] * 8) + Cells[h])) & 1)) printf("1");
            else printf(" ");
            printf(" |");
        }
        printf("\n");
...
***********************************************************************/
void
Abc_NtkShow6VarFunc(char* pF0, char* pF1) {
    word F0, F1;
    if (strlen(pF0) != 16) {
