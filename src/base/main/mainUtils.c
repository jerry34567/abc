...
***********************************************************************/
char*
Abc_UtilsGetVersion(Abc_Frame_t* pAbc) {
...
***********************************************************************/
char*
Abc_UtilsGetUsersInput(Abc_Frame_t* pAbc) {
...
***********************************************************************/
void
Abc_UtilsPrintHello(Abc_Frame_t* pAbc) {
    fprintf(pAbc->Out, "%s\n", pAbc->sVersion);
}
...
***********************************************************************/
void
Abc_UtilsPrintUsage(Abc_Frame_t* pAbc, char* ProgName) {
    fprintf(pAbc->Err, "\n");
    fprintf(pAbc->Err,
            "usage: %s [-c cmd] [-q cmd] [-C cmd] [-Q cmd] [-f script] [-h] "
            "[-o file] [-s] [-t type] [-T type] [-x] [-b] [file]\n",
            ProgName);
    fprintf(pAbc->Err, "    -c cmd\texecute commands `cmd'\n");
    fprintf(pAbc->Err, "    -q cmd\texecute commands `cmd' quietly\n");
    fprintf(pAbc->Err, "    -C cmd\texecute commands `cmd', then continue in "
                       "interactive mode\n");
    fprintf(pAbc->Err, "    -Q cmd\texecute commands `cmd' quietly, then "
                       "continue in interactive mode\n");
    fprintf(pAbc->Err, "    -F script\texecute commands from a script file and "
                       "echo commands\n");
    ...
    fprintf(pAbc->Err, "    -t type\tspecify input type (blif_mv (default), "
                       "blif_mvs, blif, or none)\n");
    fprintf(pAbc->Err, "    -T type\tspecify output type (blif_mv (default), "
                       "blif_mvs, blif, or none)\n");
...
***********************************************************************/
void
Abc_UtilsSource(Abc_Frame_t* pAbc) {
#ifdef WIN32
...
DateReadFromDateString(char* datestr) {
    ...
    if (sscanf(datestr, "%s %s %2d %2d:%2d:%2d %s %4d", day, month, &date,
        ...
        (void)sprintf(result, "%d-%3s-%02d at %d:%02d %s", date, month,
